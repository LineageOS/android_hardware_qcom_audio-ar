...
static int32_t HandleCallbackForUPD(pal_stream_handle_t* stream_handle, uint32_t event_id,
    ...
    if (event_id == EVENT_ID_GENERIC_US_DETECTION) {
        if (*event_data == 1) {
            ...
            // TODO use ultraSoundEventPut from libnotify_ultrasound to actually report the values
...
void* DelayStopThreadLoop(void* param) {
    pal_param_payload* param_payload = NULL;
    pal_ultrasoun_rampdown_param payload;
    int32_t status = 0;
    ...
    param_payload = (pal_param_payload*)calloc(
            1, sizeof(pal_param_payload) + sizeof(pal_ultrasoun_rampdown_param));
    if (!param_payload) goto exit;

    payload.rampdown_param = 2701131777;
    param_payload->payload_size = sizeof(pal_ultrasoun_rampdown_param);
    memcpy(param_payload->payload, &payload, param_payload->payload_size);

    result = pal_stream_set_param(pal_stream, PAL_PARAM_ID_ULTRASOUND_RAMPDOWN, param_payload);
    if (result != 0) {
