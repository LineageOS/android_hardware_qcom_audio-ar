...
static void* DelayStopThreadLoop(void* param) {
    pal_param_payload* param_payload = NULL;
    pal_param_ultrasound_notify payload;
    int stop_latency;
    int32_t status = 0;
    ...
    param_payload = (pal_param_payload*)calloc(
            1, sizeof(pal_param_payload) + sizeof(pal_param_ultrasound_notify));
    if (!param_payload) goto exit;

    param_payload->payload_size = sizeof(pal_param_ultrasound_notify);
